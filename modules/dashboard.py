...
def dashboard():
    ...
    styled = recent[display_cols].style.applymap(color_congestion, subset=["Congestion Level"])
    ...
    # st.title("🏛 Bengaluru Traffic Control Center")
    #
    # col1, col2, col3, col4 = st.columns(4)
    #
    # col1.metric("Total Traffic Volume", int(df["Traffic Volume"].sum()))
    # col2.metric("Avg Congestion", round(df["Congestion Level"].mean(), 2))
    # col3.metric("Total Incidents", int(df["Incident Reports"].sum()))
    # col4.metric("Avg Speed", round(df["Average Speed"].mean(), 2))
    #
    # area_congestion = df.groupby("Area Name")["Congestion Level"].mean().sort_values(ascending=False)
    #
    # st.subheader("🚨 Top Congested Areas")
    # st.dataframe(area_congestion.head())
    #
    # high_capacity = df[df["Road Capacity Utilization"] > 80]
    #
    # st.subheader("⚠ High Capacity Utilization Roads")
    # st.dataframe(high_capacity[["Area Name", "Road/Intersection Name", "Road Capacity Utilization"]])
    #
    # incident_area = df.groupby("Area Name")["Incident Reports"].sum().sort_values(ascending=False)
    #
    # st.subheader("🚑 Incident Distribution by Area")
    # st.bar_chart(incident_area)
    #
    ...
    #  claude code
    # st.write(f"Date: {now.date()} , Time: {now.time()}")
    # st.title("🏛️ Government Dashboard")
    # st.caption("Welcome back! Here's today's traffic overview for Bengaluru.")
    # # Placeholder metric cards — you'll fill these with real data later
    # c1, c2, c3, c4 = st.columns(4)
    # c1.metric("🛣️ Roads Monitored", "16", "Active")
    # c2.metric("🔴 High Congestion", "4 Roads", "+2 from yesterday")
    # c3.metric("⚠️ Active Alerts", "7", "3 new")
    # c4.metric("🚗 Avg Speed", "28 km/h", "-5 from morning")
    # st.divider()
    # st.info("📌 Use the sidebar to navigate to Prediction Model, Heat Map, or Alert Zone.")
