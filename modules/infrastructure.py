...
def _get_impact(infra_type, area_name, congestion):
    if infra_type == "Flyover / Overbridge":
        return {"congestion_reduction": "30-40%", "travel_time_improvement": "25-35%",
                "redistribution": f"Interruption-free flow through {area_name}"}
    elif infra_type == "Underpass":
        return {"congestion_reduction": "25-35%", "travel_time_improvement": "20-30%",
                "redistribution": f"Signal conflict elimination in {area_name}"}
    elif infra_type == "Road Expansion":
        return {"congestion_reduction": "20-25%", "travel_time_improvement": "15-20%",
                "redistribution": f"Increased lane capacity for {area_name}"}
...
def infrastructure_page():
    ...
    traffic_df = pd.read_csv("Bangalore_traffic_Dataset.csv")

    # Normalize columns
    traffic_df.columns = traffic_df.columns.str.strip().str.replace(" ", "_")

    # ✅ FIX: map back to original expected names (NO LOGIC CHANGE)
    traffic_df = traffic_df.rename(columns={
        "Area_Name": "Area Name",
        "Traffic_Volume": "Traffic_Volume",
        "Congestion_Level": "Congestion Level",
        "Road_Capacity_Utilization": "Road Capacity Utilization"
    })
